...
impl Mode {
    ...
    pub fn refine_vec(&self, candidates: &[String], needle: &str) -> Vec<result::Result> {
        match *self {
            Mode::Normal => {
                candidates.iter().flat_map(|s|{refine_normal_string(s, needle).into_iter()}).collect()
            },
            Mode::Regexp => vec![],
            Mode::Fuzzy => vec![],
        }
...
mod test {
    ...
    fn refine_string_test() {
        ...
        assert_eq!(Some(result::Result::new("abc", vec![1].into_iter().collect())), Normal.refine_string("abc", "b"));
        assert_eq!(Some(result::Result::new("abc", vec![0, 2].into_iter().collect())), Normal.refine_string("abc", "a c"));
        assert_eq!(Some(result::Result::new("abc", vec![0, 2].into_iter().collect())), Normal.refine_string("abc", "a  c "));
    }
}
